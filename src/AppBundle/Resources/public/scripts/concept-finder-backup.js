$(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.status == "success") {
                        ...
                        handleFindConceptResult(result.data);
    ...
    body.on("click", ".nav-to-prev-page:not(.disabled)", function (event) {
            event.preventDefault();
            showLoadingOverlay($(this), function (currentInstance) {
        ...
        .on("click", ".nav-to-next-page:not(.disabled)", function (event) {
            ...
            showLoadingOverlay($(this), function (currentInstance) {
                var dimKey = currentInstance.closest("ul").data("dim");
                var elementCount = CONCEPT_FINDER_DATA['context'][dimKey].length;
    ...
    $(".pager-input").on("change", function (event) {
        ...
        showLoadingOverlay($(this), function (currentInstance) {
            ...
            var page = parseInt(currentInstance.val());
            if (page <= 1 || page >= nrPages) {
                page = 1;
            }

            CONCEPT_FINDER_DATA['page'][dimKey] = page;
            handlePageUpdate(dimKey);
...
function handleFindConceptResult(data) {
    ...
    for (var key in data['constraints']) {
        var constraint = data['constraints'][key];
    ...
    if (data['foundConcept'] != null) {
        $(".found-concept").remove();
        var concept = createConcept(data['foundConcept']);
...
function createConcept(data) {
    ...
    for (var dimKey in dimensions) {
        ...
        if (dimensions.length == 3) {
            ...
            for (itemKey in data[dimKey]) {
                elemId = data[dimKey][itemKey];
                elemName = CONCEPT_FINDER_DATA.context[dimKey][elemId];
                lock += "&lock[]=" + elemName;
...
function findConceptResetHistory() {
...
function loadBookmark(index) {
    ...
    if (states.length != 0) {
        ...
        handleFindConceptResult(lastState);
        findConceptResetHistory();
...
function handlePageUpdate(dimKey) {
    var currentPage = CONCEPT_FINDER_DATA['page'][dimKey];
    var elementCount = CONCEPT_FINDER_DATA['context'][dimKey].length;
    var pageSize = CONCEPT_FINDER_DATA['pageSize'];
    var nrPages = parseInt((elementCount - 1) / pageSize + 1);

    var list = $("ul.pager[data-dim=" + dimKey + "]");
    list.find("input").val(currentPage);

    if (currentPage <= 1) {
        list.find(".nav-to-prev-page").addClass("disabled");
    } else {
        list.find(".nav-to-prev-page").removeClass("disabled");
    }

    if (currentPage >= nrPages) {
        list.find(".nav-to-next-page").addClass("disabled");
    } else {
        list.find(".nav-to-next-page").removeClass("disabled");
    }

    changeConceptFinderPage(dimKey, currentPage);
}

function changeConceptFinderPage(dimKey, page) {
    ...
    var limit = Math.min(page * pageSize, CONCEPT_FINDER_DATA["context"][dimKey].length - 1);
    var itemSwitch, input;

    for (var index = offset; index <= limit; index++) {
        var element = CONCEPT_FINDER_DATA["context"][dimKey][index];
...
function addOperationTooltip(operation) {
    ...
    var element = CONCEPT_FINDER_DATA['context'][dimension][index];
